...
vim.api.nvim_create_autocmd({ 'BufEnter', 'FocusGained', 'InsertLeave', 'CursorHold' }, {
    callback = function()
        ...
        if num_errors > 0 then
            ...
            vim.b.winbar_color = '%#DiagnosticSignError#'
        else
            vim.b.errors = ''
            vim.b.winbar_color = ''
        end
...
--vim.opt.winbar = [[ %{get(b:, "errors", "")}%{get(b:, "warnings", "")}%{get(b:, "hints", "")}%{get(b:, "infos", "")}%{%get(b:, "winbar_color", "")%}./%f %m%h%w%r ]]
...
vim.opt.statusline =
'  %{get(g:, "branch_name", "")}   %{get(b:, "lsp_server", "")}   %{get(b:, "errors", "")}%{get(b:, "warnings", "")}%{get(b:, "hints", "")}%{get(b:, "infos", "")} %=  %m ./%f   %{get(b:, "file_icon", "")} %Y   %c, %l/%L  '
